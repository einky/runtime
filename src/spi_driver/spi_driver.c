...
/* BCM pin numbers — keep in sync with case/docs/wiring.md and pyproject. */
...
int einky_init(einky_panel_t *p) {
    ...
    if (send_cmd(p, 0x01) < 0) return -1; /* POWER_SETTING */
    if (send_data(p, pwr, sizeof(pwr)) < 0) return -1;
    if (send_cmd(p, 0x06) < 0) return -1; /* BOOSTER_SOFT_START */
    if (send_data(p, booster, sizeof(booster)) < 0) return -1;
    if (send_cmd(p, 0x04) < 0) return -1; /* POWER_ON */
    if (wait_busy(p) < 0) return -1;

    return 0;
}

static int push_frame(einky_panel_t *p, uint8_t cmd, const uint8_t *frame, size_t len) {
    if (!p || !frame) return -EINVAL;
    if (len != EINKY_FRAME_BYTES) return -EINVAL;
    if (send_cmd(p, cmd) < 0) return -1;
    if (send_data(p, frame, len) < 0) return -1;
    if (send_cmd(p, 0x12) < 0) return -1; /* DISPLAY_REFRESH */
    return wait_busy(p);
}
...
int einky_sleep(einky_panel_t *p) {
    if (!p) return -EINVAL;
    if (send_cmd(p, 0x02) < 0) return -1; /* POWER_OFF */
    if (wait_busy(p) < 0) return -1;
    static const uint8_t deep[] = {0xa5};
    if (send_cmd(p, 0x07) < 0) return -1; /* DEEP_SLEEP */
    return send_data(p, deep, sizeof(deep));
}

void einky_close(einky_panel_t *p) {
    if (!p) return;
    if (p->spi_fd >= 0) close(p->spi_fd);
    free(p);
}
